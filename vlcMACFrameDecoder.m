...
%   [CFG, PAYLOAD] = MACFRAMEDECODER(MACFRAME) is the same as the
  ...
  persistent crc % Persistent crc decoder for performance optimization
...
%   if ~isequal(MFR, MFR2)
%     % CRC check failed. MAC frame must be rejected. Output empty structure
...
%   
  ...
  if ~strcmp(cfg.FrameType, 'Data')
    ...
    dataPayload = dec2hex(bi2de(dataPayload));
...
function cfg = decodeFrameControl(cfg, frameControl)
 ...
 % x= frameControl(7:9)
  ...
  switch bi2de(frameControl(13:14))
    ...
    %%disp "destAddr"
    %%disp (cfg.DestinationAddressing)
  end
...
%     cfg.SourcePANIdentifier = dec2hex(bi2de(macFrame(cnt : cnt+2*8-1)), 4);
%     cnt = cnt + 2*8;
%   end
...
function [cfg, varargout] = decodeMACPayload(cfg, macPayload)
  ...
  elseif strcmp(cfg.FrameType, 'Data')
    varargout{1} = macPayload(9:end); % 100% payload, no other fields
